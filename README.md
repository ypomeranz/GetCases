...
**PDF** shows the official scan in place of the text, and **Text** brings the text back at the passage the PDF was left on. Tick **Window → View PDF in Separate Window** and the scan instead opens in a small window of its own – the page under one thin strip carrying small **save** and **print** icons and the zoom controls (**−**, **+**, **Fit**, and Ctrl/Cmd +, −, 0), and nothing else: the opinion text stays on screen beside it. Citations in the page are still clickable, Ctrl/Cmd+F still searches it, and Save (Ctrl/Cmd+S), Print (Ctrl/Cmd+P) and Close (Ctrl/Cmd+W) are also on the strip’s right‑click menu. Saved and printed files are named for the case in Bluebook form – for a case you reached by following a citation, from the opinion text fetched in the background, which is what supplies the caption, the parallel citations and the decision date. The setting is remembered between sessions.
...
The **case name at the right of that strip is a link to the opinion’s text**. For a scan opened from the reader it brings that reader back to the front – or, if you have since closed it, opens it again from what the History entry keeps, with no refetch. For a case you arrived at by following a citation it opens the text the ordinary way, and the lookup runs in the background while you read the scan, so the text is already in hand when you click.

A PDF viewer is a window in its own right: **closing the opinion text leaves it open**, and Save, Print and the citations on its pages keep working.
