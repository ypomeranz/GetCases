...
**Side panel** (or press **s**) opens the case‑details panel beside the opinion: the window widens by the panel’s width so the text keeps its measure and does not re‑wrap, and narrows again when you close it. The window may end up wider than the screen – move it left if you like. A maximized window has nowhere to grow, so there the panel takes its width from the text as before.
